...
#[serde(tag = "type")]
pub enum HostConfiguration {
    #[serde(rename = "http")]
    HTTP(HTTPHostConfiguration),
}

#[derive(Debug, Deserialize)]
pub struct HTTPHostConfiguration {
    pub label: String,
    pub url: String,
...
"#;
        let host: HostConfiguration = serde_yaml::from_str(yaml).unwrap();
        match host {
            HostConfiguration::HTTP(http_host) => {
...
status: 1000
"#;
        let result: Result<HostConfiguration, _> = serde_yaml::from_str(yaml);
